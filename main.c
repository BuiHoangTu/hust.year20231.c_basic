...
int main () {
    ...
    ArrayList *lists[2];
    lists[0] = createArraylist();
    lists[1] = createArraylist();
    ...
    while (1) {
        ...
        int listId;
        int value;
        sscanf(line + 2, "%d %d", &listId, &value);
        listId -= 1;

        if (strncmp("I", line, 1) == 0) {
            arraylist_insert(lists[listId], 0, copy2heap(&value, sizeof(int)));
        }

        if (strncmp("A", line, 1) == 0) {
            arraylist_add_last(lists[listId], copy2heap(&value, sizeof(int)));
        }
    }

    int i = 0;
    while (1) {
        if (i >= lists[0]->length && i >= lists[1]->length) break;

        int idx = i % 2;
        if (lists[idx]->length > i) {
            printf("%d ", *((int*) arraylist_get(lists[idx], i)));
        }
